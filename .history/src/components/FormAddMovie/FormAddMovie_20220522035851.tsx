...
const FormAddMovie: React.FC = () => {
  ...
  const removeItemPlanetsList = (index: string) => {
    console.log(index)
/*     console.log(...planetsList.filter((item, index) => item[index] !== index))
        ...
        <article className={style()}>
          <form className={style('formAddMovie')} onSubmit={handleSubmitToLocalStorage}>
            <div>
              ...
              {
                planetsList && <PlanetsList planetsList={planetsList} remove={(index: string) => removeItemPlanetsList(index)} />
              }
              <BoxSearchInput
                label="Add Planet"
