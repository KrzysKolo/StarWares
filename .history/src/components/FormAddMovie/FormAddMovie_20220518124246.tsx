...
const FormAddMovie: React.FC = () => {
  ...
  const [planets, setPlanets] = useState<string[]>(_planets);
  ...
  useEffect(() => {
    const getPalents = async () => {
      try {
        ...
        console.log(planets);
        dispatch(getPlanetsApi(planets.data.results.name));
      }
      catch {
        console.log("error")
      }
  ...
  const handleChangePlanets = (text) => {
    setSearchText(text);
  };
  console.log(planets)
  return (
          ...
          <form className={style('formAddMovie')} onSubmit={handleSubmitToLocalStorage}>
            <div>
              ...
              <BoxSearchInput
                ...
                onChange={e=>handleChangePlanets(e.target.value)}
                photo={search}
                altPhoto="Search" />
            </div>
